...
function getEffectiveSupplier(item: {
  ...
  hasStockData: boolean; // ← TAMBAH INI
} {
...
products.post("/:id/clone", requireRole("admin", "super"), async (c) => {
  ...
  if (exists) return c.json({ error: "Product already exists" }, 409);
...
products.post("/merge", requireRole("admin", "super"), async (c) => {
  const { keepId, mergeIds }: { keepId: number; mergeIds: number[] } =
    await c.req.json();
  ...
  for (const oldId of mergeIds) {
    ...
    const seen = new Map<number, number>(); // setId → first item id
...
products.post("/import", requireRole("admin", "super"), async (c) => {
  ...
  if (!name?.trim()) return c.json({ error: "Product name required" }, 400);
  ...
  if (exists) return c.json({ error: `Product "${name}" already exists` }, 409);
      ...
      const masterItem = await prisma.item.findFirst({
        where: {
          OR: [
            { stableId: identifier },
            {
              partNumber: { equals: identifier, mode: "insensitive" as const },
            },
          ],
        },
      });
...
function extractMpnFromDescription(desc: string): string | null {
  if (!desc) return null;
...
function extractLcscCode(url: string): string | null {
  if (!url) return null;
...
products.post("/import/analyze", requireRole("admin", "super"), async (c) => {
  ...
  for (const entry of items) {
    ...
    const resolvedMpn =
      extractMpnFromDescription(description ?? "") ?? undefined;
    ...
    if (resolvedMpn && resolvedMpn !== identifier) {
      orConditions.push({
        partNumber: { equals: resolvedMpn, mode: "insensitive" as const },
      });
    ...
    if (stripped !== identifier) {
      ...
      orConditions.push({
        partNumber: { equals: stripped, mode: "insensitive" as const },
      });
    }
    if (lcscCode) {
      orConditions.push({
        stockCode: { equals: lcscCode, mode: "insensitive" as const },
      });
    ...
    if (!item) {
      const candidates = [identifier, stripped, resolvedMpn].filter(
        Boolean,
      ) as string[];
      for (const candidate of candidates) {
        const normalized = candidate.toLowerCase().replace(/[^a-z0-9]/g, "");
        if (!normalized) continue;
        const rows = await prisma.$queryRaw<{ stable_id: string }[]>`
...
products.post(
  ...
  async (c) => {
    ...
    for (const entry of items) {
      ...
      if (!identifier || !designators) continue;
      ...
      else results.notFound.push(identifier);
    }
      ...
      try {
        ...
        if (existing) {
          ...
          await prisma.productItem.update({
            ...
            data: {
              quantitySum:
                (existing.quantitySum || 0) + (Number(quantity) || 0),
...
products.get("/:id/export/bom", async (c) => {
  ...
  let tempOverrides: Record<string, Partial<Item>> = {};
  try {
    const tempOverridesStr = c.req.query("tempOverrides");
    if (tempOverridesStr) tempOverrides = JSON.parse(decodeURIComponent(tempOverridesStr));
  } catch {}
  ...
  if (uniqueAltIds.length > 0) {
    ...
    fetchedAlts.forEach((it) => altItemsCache.set(it.stableId, it as any as Item));
  }
  ...
  for (const row of bomRows) {
    ...
    let itemToExport: Item | Partial<Item> = row.item as any as Item;
    let isSwapped = false;
    ...
    if (!isTempOverride) {
      const mainSup = getEffectiveSupplier(itemToExport as any);
      ...
      let bestAltItem: Item | null = null;
      ...
      for (const altId of altIds) {
        ...
        const altSup = getEffectiveSupplier(altItem as any);
    ...
    const finalSup = getEffectiveSupplier(itemToExport as any);
