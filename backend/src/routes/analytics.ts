...
const analytics = new Hono<{ Variables: { user: AuthUser } }>()
    ...
    for (const si of set.items) {
      ...
      if (p) {
        for (const pi of p.items) {
          bomRows++
          if (!pi.item.priceMin || pi.item.priceMin <= 0) missingPrices++
        }
      }
    }
    ...
    for (const si of ss.items) {
      for (const csi of si.set.items) {
        ...
        if (p) {
          for (const pi of p.items) {
            totalRows++
            if (!pi.item.priceMin || pi.item.priceMin <= 0) missingRows++
          }
        }
      }
    }
