...
sets.get('/:id/contents', async (c) => {
    ...
    async function resolveSet(setId, depth = 0) {
        if (depth > 8)
            return new Map();
        ...
        if (!cs)
            return new Map();
        const base = cs.parentSetId ? await resolveSet(cs.parentSetId, depth + 1) : new Map();
...
sets.get('/:id/export', async (c) => {
    const id = Number(c.req.param('id'));
    const mode = (c.req.query('mode') || 'original');
    ...
    async function resolveSet(setId, depth = 0) {
        ...
        for (const item of cs.items) {
            ...
            if (op === 'remove')
                ...
                out.set(pid, (out.get(pid) ?? 0) + item.qty);
            else
                out.set(pid, item.qty);
        }
        return out;
    }
    ...
    if (!productQtyMap.size)
        return c.json({ error: 'Project is empty' }, 404);
    ...
    ws.getCell('H10').value = mode === 'combined' ? 'BOM COMBINED' : mode === 'alternative' ? 'BOM ALT' : 'BOM';
    const DATA_START_ROW = 12;
    const user = c.get('user');
    // Generic write row helper with styling
    function writeExcelRow(rowNum, data, customNo) {
    ...
    if (mode === 'original') {
        ...
        for (const [, rows] of grouped) {
            ...
            for (const row of rows) {
                writeExcelRow(currentRow, row, itemNo++);
                currentRow++;
            }
        }
    }
