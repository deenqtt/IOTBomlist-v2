...
items.get('/', async (c) => {
    ...
    const where = {
        AND: [
            ...
            supplier ? { OR: [
                    { suppliers: { contains: supplier, mode: 'insensitive' } },
                    { supplierPrices: { contains: supplier, mode: 'insensitive' } },
                ] } : {},
            pkg ? { package: { equals: pkg, mode: 'insensitive' } } : {},
        ],
    };
...
items.get('/meta', async (c) => {
    ...
    const packageWhere = {
        package: { not: null },
        ...(filterCategory ? { category: { equals: filterCategory, mode: 'insensitive' } } : {}),
    };
    ...
    const candidates = await prisma.item.findMany({
        where: {
            AND: [
                ...
                item.value ? { value: { equals: item.value, mode: 'insensitive' } } : {},
                item.package ? { package: { equals: item.package, mode: 'insensitive' } } : {},
                item.voltageRating ? { voltageRating: { equals: item.voltageRating, mode: 'insensitive' } } : {},
                item.tolerance ? { tolerance: { equals: item.tolerance, mode: 'insensitive' } } : {},
            ],
        },
        take: 20,
    });
...
function removeFromAlts(current, remove) {
    if (!current)
        return "";
    return current
        .split(/[;,]/)
...
items.post("/:stableId/alternatives/link", requireRole("admin", "super"), async (c) => {
    ...
    function addToAlts(current, add) {
        const list = (current ?? "")
            .split(";")
            .map((s) => s.trim())
            .filter(Boolean);
...
items.delete("/:stableId/alternatives/unlink", requireRole("admin", "super"), async (c) => {
    ...
    await Promise.all([
        ...allProductItemsA.map((pi) => prisma.productItem.update({
            where: {
        ...
        ...allProductItemsB.map((pi) => prisma.productItem.update({
            where: {
                productId_stableId: { productId: pi.productId, stableId: pi.stableId },
            },
...
items.post('/', requireRole('admin', 'super'), async (c) => {
    ...
    if (data.partNumber && data.manufacturer) {
        const existing = await prisma.item.findFirst({
            where: {
                partNumber: { equals: data.partNumber, mode: 'insensitive' },
...
items.delete('/:stableId', requireRole('admin', 'super'), async (c) => {
    ...
    await Promise.all([
        ...
        ...globalAffected.map(it => prisma.item.update({
        ...
        ...bomRowsAffected.map(pi => prisma.productItem.update({
