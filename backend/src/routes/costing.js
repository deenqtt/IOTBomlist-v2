...
function getEffectivePrice(item) {
    ...
    try {
        ...
        const entries = Object.values(spMap);
        // 1. Try cheapest among IN-STOCK suppliers
        const inStock = entries
            .filter((s) => s.price != null && (s.quantity_available ?? 0) > 0)
            .map((s) => s.price);
        if (inStock.length > 0) {
...
costing.get('/sets', async (c) => {
    ...
    async function resolveSetCost(setId, depth = 0) {
        if (depth > 8)
            return { products: new Map(), missing: 0 };
        const cs = allSetsForResolution.find(s => s.id === setId);
        if (!cs)
            return { products: new Map(), missing: 0 };
        const base = cs.parentSetId ? await resolveSetCost(cs.parentSetId, depth + 1) : { products: new Map(), missing: 0 };
        const productQtys = new Map(base.products);
        for (const item of cs.items) {
            const op = item.op.toLowerCase();
            if (op === 'remove')
                productQtys.delete(item.mainProductId);
            else if (op === 'add')
                productQtys.set(item.mainProductId, (productQtys.get(item.mainProductId) ?? 0) + item.qty);
            else
                productQtys.set(item.mainProductId, item.qty);
        }
        let totalCost = 0;
        let totalMissing = 0;
...
async function resolveSetProductQtys(setId) {
    // Simple version without recursion depth limit for now (matches supersets.ts logic)
    const sets = await prisma.configSet.findMany({
        include: { items: { orderBy: { orderIndex: 'asc' } } }
    });
    async function resolve(sid, depth = 0) {
        if (depth > 8)
            return new Map();
        const cs = sets.find(s => s.id === sid);
        if (!cs)
            return new Map();
        const base = cs.parentSetId ? await resolve(cs.parentSetId, depth + 1) : new Map();
