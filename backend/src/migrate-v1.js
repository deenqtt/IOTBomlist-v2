...
function deriveProductName(sourceFile) {
...
async function main() {
    ...
    const uniqueRows = XLSX.utils.sheet_to_json(uniqueWs, { defval: '' });
    ...
    const bomRows = XLSX.utils.sheet_to_json(bomWs, { defval: '' });
    console.log(`\nImporting ${bomRows.length} BOM rows from EnhancedBOM...`);
    // Collect unique products
    const productNames = [...new Set(bomRows.map(r => strOrNull(r['Source File'])).filter(Boolean).map(f => deriveProductName(f)))];
