...
http {
    ...
    # Route API calls (has Bearer token) to backend, browser navigation to frontend
    map $http_authorization $api_upstream {
        ~^Bearer   backend;
        default    frontend;
    }
    ...
    server {
        ...
        # Auth routes always go to backend (login/register don't have token yet)
        location ~ ^/auth(/|$) {
